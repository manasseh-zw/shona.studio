    ...
    return (
      <TableRow key={task.id} className="hover:bg-muted/50">
        ...
        <TableCell className="h-16 px-4 max-w-[300px] text-sm text-muted-foreground">
          <TooltipProvider>
            <Tooltip>
              <TooltipTrigger render={<span className="block cursor-help truncate" />}>{task.notes}</TooltipTrigger>
          ...
          <TooltipProvider>
            <div className="flex items-center gap-1">
              {(task.status === "pending" || task.status === "blocked") && (
                <Tooltip>
                  <TooltipTrigger render={<Button variant="outline" size="icon" className="h-8 w-8" onClick={() => handleAction(task, "start")} disabled={busy} aria-label="Start" />}>{startPending ? (
              ...
              {task.status === "in-progress" && (
                <>
                  <Tooltip>
                    <TooltipTrigger render={<Button variant="outline" size="icon" className="h-8 w-8" onClick={() => handleAction(task, "pause")} disabled={busy} aria-label="Pause" />}>{pausePending ? (
                                                              <Loader2 className="size-4 animate-spin" />
                                                            ) : (
                  ...
                  <Tooltip>
                    <TooltipTrigger render={<Button variant="outline" size="icon" className="h-8 w-8" onClick={() => handleAction(task, "complete")} disabled={busy} aria-label="Complete" />}>{completePending ? (
                                                              <Loader2 className="size-4 animate-spin" />
                                                            ) : (
              ...
              <Tooltip>
                <TooltipTrigger render={<Button variant="outline" size="icon" className="h-8 w-8 text-destructive hover:bg-destructive hover:text-white" onClick={() => handleAction(task, "delete")} disabled={busy} aria-label="Delete" />}>{deletePending ? (
                                                  <Loader2 className="size-4 animate-spin" />
                                                ) : (
              ...
              <Tooltip>
                <TooltipTrigger render={<Button variant="outline" size="icon" className="h-8 w-8" onClick={() => handleAction(task, "view")} disabled={busy} aria-label="View details" />}><FileTextIcon className="size-4" /></TooltipTrigger>
